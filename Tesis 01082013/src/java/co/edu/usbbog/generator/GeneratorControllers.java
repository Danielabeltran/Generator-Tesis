...
import org.apache.velocity.app.Velocity;
        ...
        /**
        * @author Clase Generador de Controllers
        */

public class GeneratorControllers {
        ...
        */
        Template ModeloGeneratorControllers = null;
        
        try {
            ModeloGeneratorControllers = Velocity.getTemplate("TemplatesClass/TControllers.vsl");
        } catch (Exception e) {
            System.out.println("Error creating template");
        }

        if (ModeloGeneratorControllers != null) {
            try {

                for (Entity e : model.getEntities()) {
                    contexto.put("entidad", e);
                    BufferedWriter escritor = new BufferedWriter(new FileWriter(g.myFolder() + model.getName() + "/src/java/controllers/" + e.getName() + "Servlet.java"));
                    ModeloGeneratorControllers.merge(contexto, escritor);
                    escritor.flush();
                    escritor.close();
                }
                ...
                System.out.println("Plantillas Controllers no creadas");
            }
        }
