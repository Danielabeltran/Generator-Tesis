...
{ 
    ...
    public static void generarCodigo(Model m, InfoDatabase in) throws IOException, SQLException 
    {
        ...
        String myfolder = in.getGenerarproyecto();
        System.out.println(myfolder);
        
        Functions g = new Functions();
        File a = new File(myfolder + NameDB);
        ModeloBuilder mo = new ModeloBuilder();
        String Path = mo.Path(in);
        ...
        for (int i=1; i<=2; i++)
        {
            ...
            g.ldir ( a + "/src/conf/java" +Path);
            g.ldir ( a + "/src/java/" + Path +".Data");
            g.ldir ( a + "/src/java/" + Path +".Entity");
            g.ldir ( a + "/src/java/" + Path +".Business");
            g.ldir ( a + "/src/java/" + Path +".Presentation");
             ...
             if (ConexionDB != null)
                 {
                     try 
                         {
                             BufferedWriter escritor = new BufferedWriter(new FileWriter(myfolder + NameDB + "/src/java/" + Path + ".Data/"  +  "ConexionDB.java"));
